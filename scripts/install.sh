...
main() {
  ...
  read -rp "Project root directories [$HOME/projects]: " PROJECT_ROOTS
  ...
  log_info "Add this to your shell configuration file"
  echo
  echo "export CD_PROJECT_ROOT=$PROJECT_ROOTS"
  echo
  echo "----------------------------------------"
  ...
  if [[ "$SHELL_CHOICE" == "1" || "$SHELL_CHOICE" == "3" ]]; then
    cat "$PROJECT_ROOT/scripts/shell/zsh_completion.sh"
  fi

  if [[ "$SHELL_CHOICE" == "2" || "$SHELL_CHOICE" == "3" ]]; then
    cat "$PROJECT_ROOT/scripts/shell/bash_completion.sh"
  fi
  echo
  echo "----------------------------------------"
